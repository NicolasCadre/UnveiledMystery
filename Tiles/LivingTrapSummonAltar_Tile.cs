...
{
    public class LivingTrapSummonAltar_Tile : ModTile
    {
        ...
        {
            Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 16, 48, ModContent.ItemType<Items.LivingTrapSummonAltar_Item>());
        }
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            float BossI;
            float BossJ;
            if (Main.tile[i + 2, j].TileType == ModContent.TileType<LivingTrapSummonAltar_Tile>())
                BossI = i + 21;
            else if (Main.tile[i + 1, j].TileType == ModContent.TileType<LivingTrapSummonAltar_Tile>())
                BossI = i + 20;
            else
                BossI = i + 19;

            if (Main.tile[i, j + 2].TileType == ModContent.TileType<LivingTrapSummonAltar_Tile>())
                BossJ = j + 2.5f;
            else if (Main.tile[i, j + 1].TileType == ModContent.TileType<LivingTrapSummonAltar_Tile>())
                BossJ = j + 1.5f;
            else
                BossJ = j + 0.5f;
